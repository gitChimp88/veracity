...
const getInverterVariableNames = async () => {
	try {
	  ...
	  const authString = await getBearerString(creds);
		...
		callForVariables(invertersArray, authString, variableIdURL)
...
// const getDeviceVariables = callForVariables(x, y, z)

// const getDeviceParametersForIrradiance = callForVariables(x, y, z)
...
/* const getTokenPromise = await getToken(creds);
		...
		authString= 'Bearer '.concat(await accessToken);
 */



// takes array of promises and returns array of variables
 function callForVariables(arr, authString, varUrlParam) {
